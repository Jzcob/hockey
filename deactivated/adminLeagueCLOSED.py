...
class adminLeague(commands.Cog, name="adminLeague"):
    ...
    @app_commands.command(name="remove_incomplete", description="Removes all users with incomplete rosters after registration closes.")
    @app_commands.default_permissions(administrator=True)
    async def remove_incomplete(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True)
        db_conn, cursor = None, None
        try:
            db_conn = self.db_pool.get_connection()
            ...
            # The rowcount attribute is not guaranteed to be accurate for SELECT, so we fetch all and count
            cursor.execute("SELECT user_id FROM rosters WHERE bench_one IS NULL")
            incomplete_rosters = cursor.fetchall()
            count = len(incomplete_rosters)

            if count == 0:
                await interaction.followup.send("✅ No incomplete rosters found to remove.", ephemeral=True)
                return

            cursor.execute("DELETE FROM rosters WHERE bench_one IS NULL")
            db_conn.commit()
            
            await interaction.followup.send(f"✅ Successfully removed **{count}** user(s) with incomplete rosters from the league.", ephemeral=True)
        except Exception:
            error_channel = self.bot.get_channel(config.error_channel)
            if error_channel: await error_channel.send(f"<@920797181034778655>```{traceback.format_exc()}```")
            if not interaction.is_expired(): await interaction.followup.send("An error occurred. The issue has been reported.", ephemeral=True)
    ...
    async def alert_incomplete(self, interaction: discord.Interaction):
        ...
        try:
            ...
            message = (
                "👋 **Friendly Reminder!**\n\n"
                "Your fantasy league registration is incomplete. As registration is now closed, you will be removed from the league shortly."
            )
            success_count, fail_count = 0, 0
            for user_id in user_ids:
                try:
