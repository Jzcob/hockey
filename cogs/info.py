...
class Info(commands.Cog):
    ...
    async def info(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message("Select a ticket type.", ephemeral=True)
        except:
            error_channel = self.bot.get_channel(config.error_channel)
            embed = discord.Embed(title="Error with `/info`", description=config.PrintException, color=config.color)
